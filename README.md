...
当前短片内容覆盖：加载测试假人、创建骨架、创建 IK 控制器、应用 8 姿势走路模板、播放、验证、导出、撤销和重做。
...
- 当前动作模板只支持 `Humanoid_v1` 和 `walk_cycle_8f`。

## 骨架生成方式

骨架阶段现在有两条路：

- `默认 / 映射 Humanoid_v1`：用于 IK 控制器、Walk_8F 动作模板、验证和导出。导入带骨骼的 T-Pose GLB 后，可以把标准关节点拖到导入模型骨骼上，Humanoid_v1 会吸附到导入骨骼的 T-Pose 坐标。
